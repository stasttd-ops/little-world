...
struct Response {
    ...
    static Response not_found(const std::string& msg = "{\"error\":\"not found\"}") {
        Response r;
        r.status = 404;
        r.content_type = "application/json; charset=utf-8";
        r.body = msg;
...
// Browser encodes spaces in filenames as %20.
// Decode the URL before converting it into a filesystem path.
...
inline std::string sanitizePath(const std::string& raw) {
    ...
    for (const auto& s : segs) clean += "/" + s;
    return clean.empty() ? "/" : clean;
}

inline bool isStaticAsset(const std::string& path) {
    auto dot = path.find_last_of('.');
    ...
    static const std::vector<std::string> assets = {
        ...
        "css", "js", "mjs", "json", "woff", "woff2", "ttf"
    };

    return std::find(assets.begin(), assets.end(), ext) != assets.end();
}
...
#ifdef _WIN32
            std::cerr << "socket() failed: " << socketError() << "\n";
            WSACleanup();
#else
            std::perror("socket");
#endif
...
#ifdef _WIN32
            std::cerr << "bind() failed: " << socketError() << "\n";
...
#ifdef _WIN32
            std::cerr << "listen() failed: " << socketError() << "\n";
            closeSocket(server_fd);
            WSACleanup();
#else
            std::perror("listen");
...
private:
    ...
    void handleConnection(socket_t fd) {
        ...
        if (headerEnd == std::string::npos) {
            ...
            headBlob = raw.substr(0, headerEnd);
            bodySoFar = raw.substr(headerEnd + 4);
        }

        std::istringstream headStream(headBlob);
        ...
        while (std::getline(headStream, line)) {
            ...
            if (key == "content-length") {
                try { contentLength = static_cast<size_t>(std::stoul(val)); }
        ...
        while (bodySoFar.size() < contentLength) {
            ...
            int n = recv(fd, buf, sizeof(buf), 0);
            if (n <= 0) break;
            bodySoFar.append(buf, static_cast<size_t>(n));
        }
    ...
    Response route(const Request& req) {
        // Decode the path for static files, but keep registered API paths
        // exactly as they are.
    ...
    Response serveStatic(std::string path) {
        std::string safe = sanitizePath(path);
        if (safe == "/") safe = "/index.html";

        std::string full = staticRoot_ + safe;
        std::string content;

        if (!readFile(full, content)) {
            // Never send index.html when an asset is missing. This is important
            // for MP3: otherwise the browser receives HTML instead of audio/mpeg.
            if (isStaticAsset(safe)) {
                return Response::not_found("{\"error\":\"static file not found\"}");
            }

            // SPA fallback for normal browser routes.
            std::string indexPath = staticRoot_ + "/index.html";
            ...
            return Response::not_found("{\"error\":\"file not found\"}");
        }

        Response r;
        r.status = 200;
        r.content_type = mimeFromExt(full);
        r.extra_headers["Accept-Ranges"] = "bytes";
        r.body = content;
        return r;
    }

    void sendResponse(socket_t fd, const Response& res) {
        ...
        while (sent < header.size()) {
            int chunk = static_cast<int>(std::min<size_t>(header.size() - sent, 1024 * 1024));
            int n = send(fd, header.data() + sent, chunk, 0);
            if (n <= 0) return;
        ...
        while (sent < res.body.size()) {
            int chunk = static_cast<int>(std::min<size_t>(res.body.size() - sent, 1024 * 1024));
            int n = send(fd, res.body.data() + sent, chunk, 0);
            if (n <= 0) break;
            sent += static_cast<size_t>(n);
        }
    }
